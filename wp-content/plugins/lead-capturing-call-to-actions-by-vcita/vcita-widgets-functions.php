 ...
 */
function vcita_widget_content($args=false) {
    $vcita_widget = (array) get_option(VCITA_WIDGET_KEY);
...
# Plug in to the_content to add the Contact Form widget if required
add_filter('the_content', 'vcita_the_content_filter');
function vcita_the_content_filter($content) {
  global $post;
  global $contact_page;
  ...
  if ($contact_page && $post && ($post->ID == $contact_page)) {
    $content .= do_shortcode('['.VCITA_WIDGET_SHORTCODE.']');
  }
